...
class UserModel with ChangeNotifier {
  ...
  Future<void> loginFirebaseSMS(
    ...
    try {
      ...
      print("zaid shami cookie "+user!.cookie!);
  ...
  Future<void> saveUser(User? user) async {
    try {
      ...
      loggedIn = true;
//user!.cookie=ServerConfig().accessToken;;
      // save the user Info as local storage

      await _storage.setItem(kLocalKey['userInfo']!, user);
      await _storage.setItem('opencart_cookie', user!.cookie);
 ...
 //   await _service.api.getUserInfo("");
    ...
    try {
      ...
      if (json != null) {
        ...
        if (userInfo != null) {
          ...
          await Services().firebase.getMessagingToken().then((token) {
            ...
            try {
              print('[🔽 updateDeviceToken2 $token ] user cookie ${user!.cookie}');
              Services().api.updateUserInfo({'deviceToken': token}, user!.cookie);
            } catch (e) {
              print("jijijijj"+e.toString());
            }
          });
        }
        delegate?.onLoaded(user);
      ...
      print("zzzzzzzzzzzz3 "+err.toString());
    }
  }

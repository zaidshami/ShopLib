...
class WooCommerceService extends BaseServices {
  ...
  Future<List<SubAttribute>> getSubAttributes({int? id, String? lang}) async {
    try {
      ...
      for (var i = 1; i < 100; i++) {
        var subAttributes = await getSubAttributesByPage(
          id: id,
          page: i,
          lang: lang,
        );
...
//          }
  ...
  Future<Map<String, dynamic>> updateUserInfo(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Can not update user info');
      }
    } catch (err) {
      rethrow;
    }
  ...
  Future<User?> login({username, password}) async {
    ...
    try {
      ...
      if (response.statusCode == 200 && isNotBlank(body['cookie'])) {
        ...
        throw ('The account is pending approval.');
      } else {
  ...
  Future<Map<String, Tag>> getTagsByPage({String? lang, int? page}) async {
    try {
      var url =
          'products/tags?per_page=100&page=$page&hide_empty=${kAdvanceConfig.hideEmptyTags}';
      if (lang != null && kAdvanceConfig.isMultiLanguages) {
        url += '&lang=$lang';
      }
      var response = await wcApi.getAsync(url);

      return TagModel.parseTagList(response);
    } catch (e) {
      //This error exception is about your Rest API is not config correctly so that not return the correct JSON format, please double check the document from this link https://docs.inspireui.com/fluxstore/woocommerce-setup/
      rethrow;
    }
  ...
  Future<Map<String, Tag>> getTags({String? lang}) async {
    try {
      ...
      while (!isEnd) {
        var tags = await getTagsByPage(lang: lang, page: page);
        if (tags.isEmpty) {
          isEnd = true;
        }
        page = page + 1;
  ...
  Future<PagingResponse<Category>> getSubCategories({
    ...
    try {
      var url =
          'products/categories?parent=$parentId&exclude=$kExcludedCategory&per_page=$limit&page=$page&hide_empty=${kAdvanceConfig.hideEmptyCategories}';
      if (langCode != null && kAdvanceConfig.isMultiLanguages) {
        url += '&lang=$langCode';
      }
      var response = await wcApi.getAsync(url);
      return PagingResponse<Category>(
        data: CategoryModel.parseCategoryList(response),
      );
    } catch (e) {
      //This error exception is about your RegetHomeCache(st API is not config correctly so that not return the correct JSON format, please double check the document from this link https://docs.inspireui.com/fluxstore/woocommerce-setup/
      rethrow;
